 ...
 * @param {Array} existingChartConfig
 * @returns Array
 */
export function getNewChartConfig(innerDimension, children, existingChartConfig = []) {
    ...
    // TODO: [Children – React](https://react.dev/reference/react/Children#alternatives)
    return Children.map(children, each => {
...
function yDomainFromYExtents(yExtents, yScale, plotData) {
    ...
    const realYDomain = yScale.invert ? extent(allYValues) : [...new Set(allYValues)];
