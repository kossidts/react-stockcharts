...
function resetChart(props, firstCalculation = false) {
    ...
    // TODO check plot data

    const dimensions = getDimensions(props);
    if (!dimensions.width || !dimensions.height) {
        log(`Chart Dimension incorrect! ${mayBeDeprecationMessage}`);
    }

    // TODO: Maybe throw or render an empty canvas with error message, when either
    //  - no children
    //  - no dimension or dimension.height / dimension.width isNaN
    if (!children) {
        log(`NO Children: ${mayBeDeprecationMessage}`);
...
class ChartCanvas extends Component {
    constructor(props) {
        ...
        if (typeof props.xScale !== "function") {
            this.state = { error: "xScale property is required and must be a function" };
            return this;
        }
    ...
    render() {
        if (this.state.error) {
            // Or throw an error?
            return <div>ChartError: {this.state.error}</div>;
        }
